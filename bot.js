...
const URL_BLOCK_INFO = "https://client.sentry.testnet.private.bluzelle.com:26657/abci_info?";
const URL_BLUZELLE = "https://bluzelle.com";
const URL_GENESIS = "https://client.sentry.testnet.private.bluzelle.com:26657/genesis?";
...
const URL_STATUS = "https://client.sentry.testnet.private.bluzelle.com:26657/status?";
const URL_VALIDATORS = "https://client.sentry.testnet.private.bluzelle.com:26657/validators";
const YEARS_LEASE = 0;
...
client.on("message", async (msg) => {
    try {
        if (msg.content === "!balance") {
            const sdk = await sdk_js_1.bluzelle({
                mnemonic: MNEMONIC,
                url: URL_SENTRY,
                maxGas: MAX_GAS,
                gasPrice: GAS_PRICE,
        ...
        if (msg.content === "!create") {
            const data = await fetch_json_1.fetchJson.get(URL_MINT);
        ...
        if (msg.content === "!genesis") {
            const data = await fetch_json_1.fetchJson.get(URL_GENESIS);
            ...
            const genesisEmbed = new discord_js_1.MessageEmbed()
                .setColor(EMBED_COLOR_PRIMARY)
                .setURL(URL_BLUZELLE)
                .setAuthor("author: " + AUTHOR, LOGO, URL_BLUZELLE)
                .setDescription(BOT_NAME)
        ...
        if (msg.content === "!blockinfo") {
            const data = await fetch_json_1.fetchJson.get(URL_BLOCK_INFO);
            ...
            const blockEmbed = new discord_js_1.MessageEmbed()
                .setColor(EMBED_COLOR_PRIMARY)
                .setURL(URL_BLUZELLE)
                .setAuthor("author: " + AUTHOR, LOGO, URL_BLUZELLE)
                .setDescription(BOT_NAME)
        ...
        if (msg.content === "!status") {
            const data = await fetch_json_1.fetchJson.get(URL_STATUS);
        ...
        if (msg.content === "!validators") {
            const data = await fetch_json_1.fetchJson.get(URL_VALIDATORS);
            let validator_block_height = data.result.block_height;
            let validator_total = data.result.total;
            const validatorEmbed = new discord_js_1.MessageEmbed()
                .setColor(EMBED_COLOR_PRIMARY)
                .setURL(URL_BLUZELLE)
                .setAuthor("author: " + AUTHOR, LOGO, URL_BLUZELLE)
                .setDescription(BOT_NAME)
        ...
        if (msg.content === "!10minuteslease") {
            ...
            msg.channel.send(`Writing for ${MINUTES_LEASE} minutes your address and staking address`);
            ...
            try {
                ...
                const leaseEmbed = new discord_js_1.MessageEmbed()
                    .setColor(EMBED_COLOR_PRIMARY)
                    .setURL(URL_BLUZELLE)
                    .setThumbnail(LOGO)
                    .setAuthor("author: " + AUTHOR, LOGO, URL_BLUZELLE)
                ...
                const attachment = new discord_js_1.MessageAttachment(LOGO);
